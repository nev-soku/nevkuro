  ...
  <v-container>
    <v-row>
      <v-col v-if="rows.length">
        ...
        ></v-data-table>
        <v-col></v-col>
        <v-btn @click="$router.push('/')">戻る</v-btn>
      </v-col>
    </v-row>
  </v-container>
</template>
...
export default {
  ...
  async created() {
    ...
    const response = await fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${sheetName}!${range}?majorDimension=ROWS&key=AIzaSyD2riV4l532OYCLSF7cnqpgFS1aKImdWuU`,
    );
...
//AIzaSyD2riV4l532OYCLSF7cnqpgFS1aKImdWuU